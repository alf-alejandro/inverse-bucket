"""
dashboard.py — Flask server para Polymarket Basket Reversal Bot.
Rutas:
...
def download_csv():
    ...
    return send_file(CSV_FILE, mimetype="text/csv", as_attachment=True, download_name="reversal_trades.csv")
...
#phase-badge.active  { border-color: var(--red);  color: var(--red); }
...
.kpi-label { font-size: 8px; letter-spacing: 2.5px; text-transform: uppercase; color: var(--dim); margin-bottom: 10px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.kpi-val { font-family: var(--sans); font-weight: 700; font-size: 20px; line-height: 1; color: var(--white); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.sym.active { color: var(--red); border-color: var(--red); }
...
@keyframes pulse-brd { from{box-shadow:0 0 0 rgba(255,45,85,0)} to{box-shadow:0 0 16px rgba(255,45,85,0.2)} }

.sig-main { font-family: var(--sans); font-weight: 900; font-size: 36px; letter-spacing: 2px; color: var(--dimmer); transition: color 0.3s; }
...
.sig-info .rl { color: var(--red); }
.sig-info .bl { color: var(--blue); }

.pos-wrap { border: 1px solid var(--red); padding: 14px 16px; background: rgba(255,45,85,0.025); margin-top: 12px; }
.pos-lbl { font-size: 8px; letter-spacing: 3px; text-transform: uppercase; color: var(--red); margin-bottom: 10px; display: flex; align-items: center; gap: 6px; }
...
<header>
  <div>
    <div class="logo">REVERSAL // <em>LIVE</em></div>
    <div class="logo-sub">ETH · SOL · BTC &mdash; Polymarket 5m · Contra-tendencia armónica</div>
  </div>
...
<div class="main-grid">
  ...
  <div class="panel">
    <div class="panel-head">Señal de Sobre-Extensión</div>
    <div class="panel-body" style="display:flex;flex-direction:column;gap:12px">
      ...
      <div id="sig-box" class="signal-wrap">
        ...
        <div class="sig-info" id="sig-info">Sin sobre-extensión detectada</div>
      </div>
      <div id="pos-wrap" style="display:none" class="pos-wrap">
        <div class="pos-lbl"><span class="dot"></span> Posición Abierta (Reversal)</div>
        <div id="pos-body"></div>
      </div>
    </div>
  </div>
...
<div class="panel" style="border:1px solid var(--line2)">
  <div class="panel-head">Trades del Día <span id="t-count" style="color:var(--white);font-size:10px;letter-spacing:1px"></span></div>
  <div class="panel-body" style="padding:0">
    <div class="t-wrap">
      <table>
        <thead><tr>
          <th>ID</th><th>Hora</th><th>Asset</th><th>Gap Lado</th><th>Reversal</th>
          <th>Ask</th><th>Gap bp</th><th>Secs</th>
          <th>PnL</th><th>Resultado</th><th>Salida</th>
        </tr></thead>
        <tbody id="t-body">
          <tr><td colspan="11" style="color:var(--dim);padding:18px 10px;text-align:center;font-size:10px">Sin trades aún</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
function initEquity() {
  ...
  eqChart = new Chart(ctx, {
    type: 'line',
    data: { labels: [], datasets: [{ data: [], borderColor: '#ff2d55', borderWidth: 1.5, pointRadius: 0, tension: 0.3, fill: true,
      backgroundColor: ctx => {
        const g = ctx.chart.ctx.createLinearGradient(0,0,0,160);
        g.addColorStop(0,'rgba(255,45,85,0.12)'); g.addColorStop(1,'rgba(255,45,85,0.01)'); return g;
      }
    }]},
    options: {
...
function updateKPIs(s) {
  ...
  we.className = 'kpi-val '+(wr>=50?'green':'red');
  document.getElementById('kpi-wr').className = 'kpi '+(wr>=50?'up':'down');
...
function updateSignal(s) {
  ...
  if (!s.signal_asset || div <= threshold) {
    box.className='signal-wrap';
    document.getElementById('sig-main').className='sig-main';
    document.getElementById('sig-main').textContent='––';
    document.getElementById('sig-info').innerHTML=`Gap actual: <span class="hl">${(div*100).toFixed(1)}bp</span> (umbral: ${(threshold*100).toFixed(1)}bp)`;
    return;
  }
  const side=s.signal_side||'', ew=s.entry_window;
  const rev=s.reversal_side||'';
  box.className='signal-wrap '+side+(ew?' WINDOW':'');
  document.getElementById('sig-main').className='sig-main '+side;
  document.getElementById('sig-main').textContent=`${s.signal_asset} ${side==='UP'?'▲':'▼'} → ${rev}`;
  document.getElementById('sig-info').innerHTML=`
    Sobre-extensión <span class="hl">${(div*100).toFixed(1)} bp</span> en <span class="rl">${side}</span><br>
    Entramos <span class="gl">${rev}</span> &nbsp;·&nbsp; <span class="bl">${s.consensus||'–'}</span><br>
    ${ew?'<span class="rl">⚡ VENTANA ACTIVA</span>':'<span>fuera de ventana</span>'}
  `;
}
...
function updatePosition(s) {
  ...
  const p=s.position;
  b.innerHTML=`
    <div class="pos-line"><span class="k">Asset</span><span>${p.asset}</span></div>
    <div class="pos-line"><span class="k">Gap lado</span><span class="r">${p.gap_side||'–'}</span></div>
    <div class="pos-line"><span class="k">Reversal lado</span><span class="g">${p.side}</span></div>
    ...
    <div class="pos-line"><span class="k">Gap entrada</span><span>${((p.gap_entry||0)*100).toFixed(1)} bp</span></div>
  `;
}
...
function updateEvents(s) {
  ...
  el.innerHTML=evs.map((e,i)=>{
    let c='';
    if (e.includes('WIN')||e.includes('REVERSAL')) c='ev-g';
...
function updateTrades(s) {
  ...
  if (!trades.length) {
    tbody.innerHTML='<tr><td colspan="11" style="color:var(--dim);padding:18px 10px;text-align:center;font-size:10px">Sin trades aún</td></tr>';
    return;
  }
  tbody.innerHTML=trades.map(t=>{
    ...
    return `<tr class="${t.outcome==='WIN'?'r-win':'r-loss'}">
      ...
      <td>${t.asset}</td>
      <td><span class="tag tag-${(t.gap_side||'').toLowerCase()}">${t.gap_side||'–'}</span></td>
      <td><span class="tag tag-${(t.reversal_side||t.side||'').toLowerCase()}">${t.reversal_side||t.side||'–'}</span></td>
